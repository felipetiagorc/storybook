...
//quantas paginas mostrar, antes dos 3 pontinhos?
/*
quantidadePgs | paginaAtiva | como fica a paginação
5                   1           [1,2,3,4,5]
5                   2           [1,2,3,4,5]
5                   3           [1,2,3,4,5]
5                   4           [1,2,3,4,5]
...
test('pagination({total:"abc", activePage:1}) should throw an error', ()=>{
    ...
    const result = 'total deve ser um número'
    try {
        pagination(params)
    } catch (e) {
...
test('pagination({total:10, activePage:"1a"}) should throw an error', ()=>{
    ...
    const result = 'activePage deve ser um número'
    try {
        pagination(params)
    } catch (e) {
        expect(e.message).to.be.equal(result)        
...
/// todas de cima são iguais se a de baixo funcionar:
